...
describe("Minter", function () {
  ...
  let noPermission: SignerWithAddress;
  let addedPermission: SignerWithAddress;
  let minterAdmin: SignerWithAddress;
  let mintSigner: SignerWithAddress;
  ...
  beforeEach(async function () {
    [deployer, minter, noPermission, addedPermission, minterAdmin, mintSigner, payee0, payee1] = await ethers.getSigners();

    baseTokenInstance = await (new BaseToken__factory(deployer)).deploy(name, symbol);
    await baseTokenInstance.deployed();

    minterInstance = await (new Minter__factory(deployer)).deploy(
    ...
    await minterInstance.grantRole(await minterInstance.ADMIN_ROLE(), minterAdmin.address);
    await baseTokenInstance.grantRole(await baseTokenInstance.MINTER_ROLE(), minterInstance.address);
  ...
  describe("Constructor", async function () {
    ...
    it("should record correct token", async function() {
      expect(await minterInstance.tokenContract()).to.be.equal(baseTokenInstance.address);
    });
    ...
    it("should record correct shares", async function() {
      expect(await minterInstance.shares(await minterInstance.payee(0))).to.be.equal(shares[0]);
      expect(await minterInstance.shares(await minterInstance.payee(1))).to.be.equal(shares[1]);
    });

    it("should set deployer as default admin", async function() {
      expect(await minterInstance.hasRole(await minterInstance.ADMIN_ROLE(), minterAdmin.address))
      .to.be.true;
    });
    ...
    describe("signedMint", function() {
      ...
      beforeEach(async function() {
        expect(await minterInstance.signedMintIsActive())
          .to.be.false;
      });

      it("should fail if signedMint is not active", async function() {
        await expect(minterInstance.signedMint(1, 1, "0x00", ethers.utils.id("anything")))
      ...
      describe("when status is active", function() {
        ...
        it("should fail if number of tokens exceeds permitted max", async function() {
          await expect(minterInstance.signedMint(4, 1, "0x00", ethers.utils.id("anything")))
        ...
        describe("when valid signature", function() {
          ...
          beforeEach(async function() {
            await minterInstance.connect(minterAdmin).setMintSigner(mintSigner.address);
            expect(await minterInstance.mintSigner()).to.be.equal(mintSigner.address);
          });
          ...
          describe("when price set", function() {
            ...
            it("should refund if too much ether sent", async function() {
              ...
              const postBalance = await ethers.provider.getBalance(minter.address);
  
              expect(preBalance.sub(postBalance).sub(gasFee)).to.be.equal(ethers.BigNumber.from(price).mul(5));
            });
            ...
            it("should mint to minter", async function() {
              ...
              expect(await baseTokenInstance.totalMinted()).to.be.equal(numberToMint);
              expect(await baseTokenInstance.totalSupply()).to.be.equal(numberToMint);
            });
            ...
            describe("maxPurchaseBehavior", function() {
              ...
              beforeEach(async function() {
                await minterInstance.connect(minterAdmin).setMaxBlockPurchase(maxPerBlock);
                await minterInstance.connect(minterAdmin).setMaxWalletPurchase(maxPerWallet);
              });
    ...
    describe("mint", function() {
      ...
      it("should fail if sale is not active", async function() {
        await expect(minterInstance.mint(1))
          .to.be.revertedWith("Minter: Sale is not active");
      });

      describe("when status is active", function() {
        beforeEach(async function() {
          await minterInstance.connect(minterAdmin).flipSaleState();
          expect(await minterInstance.saleIsActive())
            .to.be.true;
        });

        it("should fail if number of tokens is 0", async function() {
          await expect(minterInstance.mint(0))
            .to.be.revertedWith("Minter: numberOfTokens is 0");
        });
        ...
        describe("when price set", function() {
          ...
          it("should refund if too much ether sent", async function() {
            ...
            const res = await minterInstance.mint(5, { value: ethers.utils.parseEther("1") });
            const receipt = await res.wait();
            const gasFee = receipt.cumulativeGasUsed.mul(receipt.effectiveGasPrice);
          ...
          it("should fail if numberOfTokens surpasses maxSupply", async function() {
            const numberToMint = maxSupply as number + 1;
            await expect(minterInstance.mint(numberToMint, { value: ethers.BigNumber.from(price).mul(numberToMint)}))
              .to.be.revertedWith("Minter: Purchase would exceed max supply");
          });
          ...
          it("should mint to minter", async function() {
            ...
            expect(await baseTokenInstance.balanceOf(deployer.address)).to.be.equal(numberToMint);
            expect(await baseTokenInstance.totalMinted()).to.be.equal(numberToMint);
            expect(await baseTokenInstance.totalSupply()).to.be.equal(numberToMint);
          });
          ...
          describe("maxPurchaseBehavior", function() {
            ...
            const numberToMint = 11;
            const totalCost = ethers.BigNumber.from(price).mul(numberToMint);
              await expect(minterInstance.mint(numberToMint, { value: totalCost }))
                .to.be.revertedWith("Minter: maxBlockPurchase exceeded");
            });
  
            it("should fail if mint two times in a block", async function () {
              const mintTwiceInstance: MintTwiceInBlock = await (new MintTwiceInBlock__factory(deployer)).deploy();
              await mintTwiceInstance.deployed();
  
              const numberToMint = 2;
              const totalCost = ethers.BigNumber.from(price).mul(numberToMint);
  
              await expect(mintTwiceInstance.mintTwoTimes(minterInstance.address, totalCost, numberToMint, { value: totalCost.add(totalCost) }))
            ...
            it("should fail if mint too many per wallet", async function() {
              const numberToMint = 10;
              const totalCost = ethers.BigNumber.from(price).mul(numberToMint);
              await (await minterInstance.mint(numberToMint, { value: totalCost })).wait();
              await (await minterInstance.mint(numberToMint, { value: totalCost })).wait();
  
              await expect(minterInstance.mint(1, { value: price }))
                .to.be.revertedWith("Minter: Sender reached mint max");
            });
            ...
            it("should permit up to wallet minting limit", async function() {
              ...
              expect(await baseTokenInstance.balanceOf(deployer.address)).to.be.equal(numberToMint + numberToMint);
            });
          });
    ...
    describe("reserveTokens", function() {
      it("should fail if called by non admin", async function() {
        await expect(minterInstance.connect(noPermission).reserveTokens(10))
      ...
      it("should mint tokens to admin", async function() {
        ...
        expect(await baseTokenInstance.balanceOf(minterAdmin.address)).to.be.equal(10);
      });
    });
